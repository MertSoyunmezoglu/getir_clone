import React from "react";
import { NavLink } from "react-router-dom";
import { BsSearch } from "react-icons/bs";
import { FcHome } from "react-icons/fc";
import { MdOutlineArrowForwardIos } from "react-icons/md";
import PaymentOptions from "./PaymentOptions";
import { ContextProvider } from "context/GlobalContext";
import { GrLocation } from "react-icons/gr";
import { openModal } from "store/modal";
import store from "store";
import { HiPlus } from "react-icons/hi";
import PaymentSummary from "./PaymentSummary";
export default function Payment() {
  const handleMapsModal = () => {
    store.dispatch(openModal({ name: "adres" }));
  };
  const { address } = ContextProvider();
  return (
    <div>
      <div className="bg-primary-brand-color  ">
        <div className=" w-full    container">
          <div className=" w-full  flex justify-between items-center h-[4.5rem] bg-primary-brand-color ">
            <div>
              <NavLink to="/kategoriler">
                <button
                  type="submit"
                  className="   flex my-3   rounded-t-md  ml-20 "
                >
                  <img
                    className="w-16"
                    src="data:image/svg+xml;base64,"
                  />
                </button>
              </NavLink>
            </div>

            <div className="  items-center mr-28 z-10 flex">
              <form>
                <div class="relative">
                  <div class="flex absolute inset-y-0 right-16 items-center   pointer-events-none">
                    <button className="relative flex gap-2 cursor-pointer">
                      <FcHome size={20} /> Ev
                      <MdOutlineArrowForwardIos size={24} color="#5d3ebc" />
                    </button>
                  </div>
                  <span
                    type="search"
                    id="default-search"
                    class="block w-28 h-12 mr-12 text-sm text-gray-600 bg-gray-50 rounded-l-xl rounded-r-3xl border-none    "
                  />

                  <div className="">
                    <button
                      type="submit"
                      class="absolute  -z-10  top-0 -right-12  w-32 h-12 text-sm font-semibold text-white bg bg-brand-yellow rounded-r-lg   focus:ring-4 focus:outline-none  "
                    >
                      <span class=" ml-8 tracking-wider text-primary-brand-color">
                        TVS 10 dk
                      </span>
                    </button>
                  </div>
                </div>
              </form>
            </div>
          </div>
        </div>
      </div>
      <div className=" flex  container   col-2 justify-between w-full h-full ">
        <div className="ml-20 w-[48rem] ">
          Adres
          <div className=" bg-white h-auto rounded-lg  ">
            <div className=" m-4 flex   ">
              <div className=" flex   w-full h-full overflow-hidden truncate m-4">
                {address ? (
                  <div className=" flex gap-2 text-sm text-gray-600">
                    <GrLocation size={24} /> {address}
                  </div>
                ) : (
                  <button
                    onClick={handleMapsModal}
                    className=" text-sm  opacity-70 flex gap-4 font-semibold   items-center bg-white shadow-md m-2 w-8 h-8   border border-gray-200 rounded-md"
                  >
                    <span><HiPlus className="m-2 text-primary-brand-color" /></span>

                    <span>Ev adresi ekle</span>
                  </button>
                )}
              </div>
            </div>
          </div>
          Not Ekle
          <div className=" bg-white h-40 rounded-lg flex ">
            <textarea
              className=" w-96 border-2 m-8 h-24 border-gray-300 hover:border-primary-brand-color peer peer-focus:border-primary-brand-color rounded-md"
              type="textarea"
              placeholder="Sipariş notunuzu buraya yazabilirsiniz."
            />
            <span className="mt-8 mx-4 text-gray-400">
              <input
                type="checkbox"
                className="  hover:border-secondary-brand-color transition-all mr-2 w-6 h-6 rounded-md border-2 border-gray-300"
              />
              Zili Çalma
            </span>
          </div>
          Ödeme Yöntemi
          <div className=" bg-white  h-auto rounded-lg flex ">
            <PaymentOptions />
          </div>
        </div>
        <div className=" w-[24rem] mr-20  ">
          Ödeme Özeti
          <><PaymentSummary/></>
        </div>
      </div>
    </div>
  );
}
