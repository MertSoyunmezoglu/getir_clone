    ...
    <div className=' absolute inset-0  sm:px-8 justify-end mr-16 flex items-center'>
        <div className="   rounded-lg text-center w-[395px] pt-5  h-[365px] mx-12 bg-gray-50  ">
          ...
          <div className='justify-between absolute flex flex-row mt-2 ml-2  ' >
            ...
            <div className=' font-semibold text-sm opacity-60 mx-2 mt-1'>veya</div>
            <div className=' flex border-t-2 border-primary-brand-color opacity-10  w-44   mt-3 '></div>
          </div>
          
           <h4 className="text-primary-brand-color font-semibold text-center mt-12  text-md ">Giriş Yap veya Kayıt Ol</h4>
          ...
          <input required type="text" maxLength={11} value={inputValue} onChange={e => handleInput(e)} className=  {` flex-nowrap border-2 peer  hover:border-primary-brand-color   border-gray-300  rounded-md h-14 w-[252px]   ${ inputValue.length<11 ?'focus:border-red-400 ' :'null' } `}    />
          {inputValue.length>=1  && inputValue.length<11 ? <span className=' absolute text-red-600 top-4 right-0 h-full px-4'>
          <CgDanger size={24}/>
      </span>:null}
          <span className=' absolute -top-1 left-0 h-full px-2 text-sm text-gray-500 flex items-center text-md peer-focus:h-5  ml-2 peer-focus:mt-1 peer-valid:mt-1 peer-focus:text-xs transition-all peer-valid:h-5 peer-valid:text-xs peer-valid:text-secondary-brand-color'>Telefon Numarası</span>
        </label>
        
      </div>
      {inputValue.length>=1  && inputValue.length<11 ? <span className='ml-24 text-xs text-red-600'>Lütfen geçerli bir telefon numarası giriniz.</span> : null}
    <button    type="submit"  className= ' bg-brand-yellow ml-5 mt-2 h-12 text-secondary-brand-color hover:text-brand-yellow hover:bg-secondary-brand-color w-[360px] flex items-center transition-all justify-center rounded-md font-semibold text-md font-sans' >Telefon numarası ile devam et </button>
        </div>
    </div>
